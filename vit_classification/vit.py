...
class ViT(nn.Module):
    ...
    def patchify(self, images):
        ...
        patches = images.view(images.size(0), -1, 3*self.patch_dim*self.patch_dim)
        return patches
    ...
    def forward(self, images):
        ...
        cls_token_reshaped = self.cls_token.expand(-1, patches_embedded.shape[1], -1)
        output = torch.cat([cls_token_reshaped, patches_embedded]) # TODO (append a CLS token to the beginning of the sequence of patch embeddings)
        ...
        output = self.fc(output) # TODO (take the embedding corresponding to the [CLS] token and feed it through a linear layer to obtain the logits for each class)
