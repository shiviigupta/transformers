...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        if attn_mask is not None:
            # convert attn_mask which is multiplicative, to an additive mask
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = torch.einsum('nhsd, nhtd->nhst', query, key) / (self.embed_dim ** 0.5)
        ...
        w = torch.softmax(dot_product, dim=-1)
        w = self.dropout(w)
        y = torch.einsum('nhst, nhtd->nhsd', w, value)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        pos = torch.arange(0, S).unsqueeze(0).expand(N, -1)
        pos = self.encoding(pos)
        output = x + pos
...
class CrossAttentionBlock(nn.Module):
    ...
    def __init__(self, input_dim, num_heads, dropout=0.1):
        ...
        self.cross_attn = MultiHeadAttentionLayer(input_dim, num_heads=num_heads, dropout=dropout)
        self.dropout = nn.Dropout(p=dropout)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, input_dim, num_heads, dim_feedforward=2048, dropout=0.1 ):
        ...
        self.mlp = nn.Sequential(nn.Linear(input_dim, dim_feedforward), nn.ReLU(), nn.Dropout(p=dropout), nn.Linear(dim_feedforward, input_dim))
        self.dropout = nn.Dropout(p=dropout)
        self.norm = nn.LayerNorm(input_dim)
